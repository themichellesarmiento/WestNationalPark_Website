...
class Animal {
  constructor(name, description, food, group, img, url) {
    this.name = name;
...
const echidna = new Animal('Echidna', description(`Echidnas, also called spiny anteaters, are walking contradictions. They are mammals,but they lay eggs.
  ...
  egg - laying mammals.The only other living monotreme is the platypus`),
  `Insects such as ants and termites, beetle larvae and worms.`, 'mammal', './assets/animals/mammals/mammals-echidna.png', 'mammals.html');

const tasmanianDevil = new Animal('Tasmanian Devil', description(`The Tasmanian devil (Sarcophilus harrisii) (palawa kani: purinina)[3] is a carnivorous marsupial of the family Dasyuridae. Until recently, it was only found on the island state of Tasmania,
  ...
  Tasmanian devil's large head and neck allow it to generate among the strongest bites per unit body mass of any extant predatory land mammal. It hunts prey and scavenges on carrion.`), `A predator, then eat meat from other animals such as wallabies and wombats`, 'mammal', './assets/animals/mammals/mammals-tasmanian_devil.png', 'mammals.html');

const quokka = new Animal('Quokka', description(`The Quokka, Setonix brachyurus, was described by early Dutch explorer, Willem de
...
cover its claws.`), `Plant eaters, they munch on shrubs and grasses`, 'mammal', './assets/animals/mammals/mammals-quokka.png', 'mammals.html');

const frillNeckedLizard = new Animal('Frill-Necked-Lizard', description(`When this unique creature feels threatened, it rises on its hind legs, opens its yellow-
...
approximately 80 cm in length.`), `other animals (sponges & jellyfish), sea plants`, 'reptile', './assets/animals/reptiles/reptiles-hawsksbill_turtle.png', 'reptiles.html');

const perentie = new Animal('Perentie', description(`The perentie (Varanus giganteus) is the largest monitor lizard or goanna native to
...
ceremonial purposes.`), `Carnivore, they eat animals like kangaroos, rabbits, lizards and birds`, 'reptile', './assets/animals/reptiles/reptiles-perentie.png', 'reptiles.html');

const cassowary = new Animal('Cassowary', description(`It is not hard to imagine that cassowaries are descended from dinosaur ancestors. The
...
Florida`), `Plants matter like fruit, insects and small animals like mice amd lizards`, 'bird', './assets/animals/birds/birds-cassowary.png', 'birds.html');

const kookaburra = new Animal('Kookaburra', description(`Native to the eucalyptus forests of eastern Australia, the laughing kookaburra is the
...
face and its upper bill is black. Its reddish-coloured tail is patterned with black bars.`), `Insects and smalll animals including snakes, frogs and lizards`, 'bird', './assets/animals/birds/birds-kookaburra.png', 'birds.html');

const yellowTailedBlackCockatoo = new Animal('Yellow Tailed Black Cockatoo', description(`It has a short crest on the top of its head. Its plumage is mostly brownish black and it
...
and Tasmania) have more prominent scalloping.`), `Fruit, seeds and other plant material`, 'bird', './assets/animals/birds/birds-blackCockatoo.png', 'birds.html');
